...
const MusicClipSelector = ({ onClipSelect, selectedClip }) => {
  ...
  const getClipInfo = (filename) => {
    ...
    if (match) {
      ...
      return { chatId, messageId, isLyriaClip: true };
    }
    return { isLyriaClip: false };
  };
    ...
    return (
        <div className="clip-selector">
          ...
          <div className="error-message">
            <p>{error}</p>
            <button onClick={fetchMusicFiles} className="retry-button">Retry</button>
          </div>
        </div>
    );
  }
  ...
  return (
      <div className="clip-selector">
        <div className="selector-header">
          <h2>Music Clips</h2>
          <button onClick={fetchMusicFiles} className="refresh-button">↻ Refresh</button>
        </div>
